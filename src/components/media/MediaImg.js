...
const MediaImg = ({ id,size }) => {
  ...
  if (!image) return <img src="https://placehold.co/600x400?text=Loading..." alt="pristojni alternativni tekst"/>;

  const imgSize = image?.media_details?.sizes[size]?.source_url || image?.guid?.rendered; // ukoliko nema zadane veličine uzima guid rendered sliku, jer u ekstremnim slučajevima nema uopće veličine

  return (

    <img src={imgSize  || "/img/post-village.webp"} alt={image.alt_text || "alt text"} />
